...
Приложение должно представлять собой HTTP-сервер, реализующий необходимый [API](./api.yml). В наследие от предыдущей команды вам достались инстансы PostgreSQL и Redis, которые вы можете использовать для хранения данных (что именно использовать &mdash; решаете вы сами).
...
подключаться к существующим СУБД и сервису антифрода. Тестирующая система самостоятельно запустит
PostgreSQL, Redis и другие зависимости.
...
Генерируемый токен должен уникально идентифицировать компанию и быть сложным для подбора (можно использовать JWT).
...
Если поле таргетинга не указано, компания решила не накладывать ограничения на пользователей по данному признаку.
...
- Далее ознакомьтесь с концептом авторизации и аутентификации.
